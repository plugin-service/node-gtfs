...
async function getTripIdsFromStoptimes(config, stoptimeQuery) {
...
exports.getRoutes = async (config, query = {}, fields = [], orderBy = []) => {
  ...
  if (Object.values(stoptimeQuery).length > 0) {
    const tripIds = await getTripIdsFromStoptimes(config, stoptimeQuery);
    ...
    const tripWhereClause = tripIds.map(tripId => `trip_id = ${sqlString.escape(tripId)}`).join(' OR ');
